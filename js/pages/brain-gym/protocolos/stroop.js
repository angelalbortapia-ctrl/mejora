/** Protocolo Stroop — BaseProtocol completo */
...
function stroopAnswer(name) {
  const {
    brainState, clearTrialDeadline, markTrial, playTone, render, ensureClinicalLab,
  } = getProtocolContext()
...
function mountStroopGlobals() {
  if (!window.stroopAnswer) window.stroopAnswer = stroopAnswer
  registerTimedHandler('stroop', () => stroopAnswer('__timeout__'))
}
