...
import { forgeSparkAt, pulseElement, haptic } from '../fx.js?v=141'
...
export function renderMejora() {
  ...
  const habitsContent = `<div class="ds-toolbar">
      <h3 class="ds-toolbar-title">Hábitos de hoy</h3>
      <button onclick="editingHabits=true;render(true)" class="btn-secondary text-sm">✏️ Editar</button>
    </div>
  ...
  const habitsChart = !editingHabits && habits.length
    ? `<div class="mejora-week-chart">${habitChartHTML()}</div>`
    : ''
  const habitsBody = habitsChart
    ? `<div class="mejora-habits-layout">${habitsChart}<div class="mejora-habits-main">${habitsContent}</div></div>`
    : habitsContent
  ...
  return `<div class="animate-fade-in page-shell page-wide page-mejora">
    <div class="ds-page ds-page--full">
      <div class="mejora-tab-panel ds-panel ds-panel--flat">
        ${pageLead('Marca cada hábito al completarlo · XP y racha por hábito')}
        ${habitsBody}
      </div>
    </div>
  </div>`
  ...
  window.adjustHabit = function(id, delta) {
    ...
    if (delta > 0) {
      ...
      if (result?.completed) {
        ...
        forgeSparkAt(row?.closest('.habit-card, .habit-item') || row)
        pulseElement(row?.closest('.habit-card, .habit-item') || row)
  ...
  window.toggleHabit = function(id) {
    ...
    if (isHabitComplete(habit)) uncompleteHabit(id)
      ...
      if (result?.completed) {
        haptic(20)
        playHabitDone()
        const row = document.querySelector(`[data-habit-id="${id}"]`) || document.querySelector(`[onclick*="toggleHabit('${id}')"]`)
        forgeSparkAt(row?.closest('.habit-card, .habit-item') || row)
        pulseElement(row?.closest('.habit-card, .habit-item') || row)
        awardXp('discipline', result.xp, result.name)
        processPlanAwards(checkPlanTask('habit'))
